...
if [ "$BASIC" = true ] ; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh open3d xatlas pyvista pymeshfix igraph transformers scikit-image einops safetensors
...
if [ "$DEMO" = true ] ; then
    pip install gradio==4.44.1 gradio_litmodel3d==0.0.1  gradio-image-prompter
    mkdir -p ./tmp/extensions
    git clone https://github.com/facebookresearch/segment-anything-2.git ./tmp/extensions/segment-anything-2
    pip install ./tmp/extensions/segment-anything-2
fi
