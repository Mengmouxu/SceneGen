...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.init_global_params = init_global_params
        ...
        self.vggt_mixin = "VGGT" in self.dataset.__class__.__name__
        if self.vggt_mixin:
            self.squeeze_data = ["x_0", "cond", "vggt_feature", "positions", "mask_cond"]
        ...
        elif finetune_ckpt is not None:    # finetune from a checkpoint right here
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        Prepare dataloader.
        """
        if "VGGT" not in self.dataset.__class__.__name__:
            ...
            self.data_sampler = DynamicBatchSampler(
                self.dataset,
                batch_size=self.batch_size_per_gpu,
                drop_last=True,
                shuffle=True,
            ...
            self.data_iterator = CyclicLoader(
                self.dataloader,
                start_epoch=self.step * self.world_size // len(self.dataloader),
            )
    ...
    @torch.no_grad()
    def visualize_scene(self, x_0, positions=None):
        """
        Convert a scene to an image.
        """
        if hasattr(self.dataset, 'visualize_scene'):
            return self.dataset.visualize_scene(x_0, positions)
        else:
            return x_0, positions

    @torch.no_grad()
    def snapshot_dataset(self, num_samples=16):
        """
        ...
        if self.vggt_mixin:
            vis_scene = self.visualize_scene(data)
        ...
        if self.vggt_mixin:
            if isinstance(vis_scene, dict):
                save_cfg = [(f'dataset_scene_{k}', v) for k, v in vis_scene.items()]
            else:
                save_cfg = [('dataset_scene', vis_scene)]
            for name, image in save_cfg:
                utils.save_image(
                    image,
                    os.path.join(self.output_dir, 'samples_scene', f'{name}.jpg'),
                    nrow=int(np.sqrt(num_samples)),
                    normalize=True,
                    value_range=self.dataset.value_range,
                )
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=16, batch_size=4, verbose=False):
        """
        ...
        num_samples = int(batch_size * self.world_size)
        # Assign tasks
        ...
        if self.vggt_mixin:
            ...
            if "sample_gt" and "pos_gt" in samples:
                sample_x_0 = samples["sample_gt"]['value']
                sample_pos = samples["pos_gt"]['value']
                vis = self.visualize_scene(x_0 = sample_x_0, positions = sample_pos)
                samples['scene_gt'] = {'value': vis, 'type': 'image'}
            if "sample" and "pos" in samples:
                sample_x_0 = samples["sample"]['value']
                sample_pos = samples["pos"]['value']
                vis = self.visualize_scene(x_0 = sample_x_0, positions = sample_pos)
                samples['scene'] = {'value': vis, 'type': 'image'}
        ...
        # Gather results
        sample_keys = list(samples.keys()).copy()
        if self.world_size > 1:
            for key in sample_keys:
                if isinstance(samples[key]['value'], torch.Tensor):
                    ...
                    if self.is_master:
                        if self.vggt_mixin and torch.cat(all_images, dim=0).shape[0] > num_samples:
                            all_images = torch.cat(all_images, dim=0)
                            scene_images = [all_images[i:i+1] for i in range(len(all_images)) if i % (batch_size + 1) == batch_size]
                            instance_images = [all_images[i:i+1] for i in range(len(all_images)) if i % (batch_size + 1) != batch_size]
                            samples[key]['value'] = torch.cat(instance_images, dim=0)[:num_samples]
                            if scene_images:  # Check if scene_images is not empty
                                samples[f'{key}_scene'] = {'value': torch.cat(scene_images, dim=0), 'type': 'image'}
                        else:
        ...
        if self.is_master:
            ...
            for key in samples.keys():
                ...
                elif samples[key]['type'] == 'position':
                    positions = samples[key]['value']
