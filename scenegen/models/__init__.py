...
def from_pretrained(path: str, **kwargs):
    ...
    print(f"Loading model from {'local' if is_local else 'Hugging Face'} path: {path}")
    if is_local:
        ...
        if os.path.exists(f"{path}.safetensors"):
            ...
            # Need to update loading mechanism for .pt files
            original_load_file = load_file
            load_file = lambda f: torch.load(f)
    else:
